    ...
    try {
        switch (name) {
            ...
            case 'huawei_upload_apk': {
                ...
                // Upload file
                const fileUrl = await huaweiClient.uploadFile(
            ...
            case 'huawei_set_test_account': {
                const { appId = HUAWEI_APP_ID, account, password, remark } = args as {
                    appId?: string;
                    account: string;
                    password: string;
                    remark?: string;
            ...
            case 'huawei_cloud_test_create': {
                ...
                const result = await huaweiClient.createCloudTestTask(
                    appId,
                    testType,
                    fileUrl,
                    testDeviceIds,
            ...
            case 'huawei_cloud_test_list': {
                const { appId = HUAWEI_APP_ID, pageNum = 1, pageSize = 10 } = args as {
                    appId?: string;
                    pageNum?: number;
                    pageSize?: number;
                };
